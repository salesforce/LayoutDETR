...
class EncoderEpilogue(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        out_channels,                   # Dimensionality of output embedding.
        resolution,                     # Resolution of this block.
        img_channels,                   # Number of input color channels.
        architecture        = 'resnet', # Architecture: 'orig', 'skip', 'resnet'.
        activation          = 'lrelu',  # Activation function: 'relu', 'lrelu', etc.
...
class Encoder(torch.nn.Module):
    def __init__(self,
        ...
        if 2**self.img_resolution_log2 < self.img_resolution:
            self.img_resolution_log2 += 1
            self.img_resolution = int(2**self.img_resolution_log2)
        self.img_channels = img_channels
        self.block_resolutions = [2 ** i for i in range(self.img_resolution_log2, 2, -1)]
        channels_dict = {res: min(channel_base // res, channel_max) for res in self.block_resolutions + [4]}
        fp16_resolution = max(2 ** (self.img_resolution_log2 + 1 - num_fp16_res), 8)
        ...
        for res in self.block_resolutions:
            ...
            setattr(self, f'b{res}', block)
            cur_layer_idx += block.num_layers
        self.b4 = EncoderEpilogue(channels_dict[4], out_channels=self.out_channels, resolution=4, **epilogue_kwargs, **common_kwargs)
...
@persistence.persistent_class
class DecoderMappingNetwork(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality, 0 = no latent.
        w_dim,                      # Intermediate latent (W) dimensionality.
        num_ws,                     # Number of intermediate latents to output, None = do not broadcast.
    ...
    def extra_repr(self):
        return f'z_dim={self.z_dim:d}, w_dim={self.w_dim:d}, num_ws={self.num_ws:d}'
...
class Decoder(torch.nn.Module):
    def __init__(self,
        ...
        img_resolution,             # Output resolution.
        img_channels,               # Number of output color channels.
        use_noise,                  # Enable spatial noise input?
        mapping_kwargs      = {},   # Arguments for MappingNetwork.
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
    ):
        super().__init__()
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, use_noise=use_noise, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = DecoderMappingNetwork(z_dim=z_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
