...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=8, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    for samples, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        ...
        images = save_real_image_with_background(bbox_real, bbox_real, bbox_patch_orig, ~padding_mask, background_orig,
                                                 out_path=None, W_page=W_page, H_page=H_page, return_instead_of_save=True)
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=8, data_loader_kwargs=None, max_items=None, batch_gen=None, **stats_kwargs):
    dataset = dnnlib.util.construct_class_by_name(**opts.dataset_kwargs)
    if data_loader_kwargs is None:
        data_loader_kwargs = dict(pin_memory=True, num_workers=3, prefetch_factor=2)
    ...
    for samples, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        ...
        images = save_real_image_with_background(bbox_fake, bbox_real, bbox_patch_orig, ~padding_mask, background_orig,
                                                 out_path=None, W_page=W_page, H_page=H_page, return_instead_of_save=True)
        images = (images * 255.0).to(torch.uint8)
        features = detector(images.to(opts.device), **detector_kwargs)
        stats.append_torch(features, num_gpus=opts.num_gpus, rank=opts.rank)
